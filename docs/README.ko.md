...
<h3 align="center">
  ...
  <code>agentcrow init</code> → <code>claude</code> → 자동 디스패치.
</h3>
...
```bash
agentcrow init                # 에이전트 + CLAUDE.md 설정 (현재 프로젝트)
agentcrow init --global       # 한번 설정, 모든 프로젝트에서 작동
agentcrow init --lang ko      # 한국어 템플릿
agentcrow init --max 5        # 디스패치당 최대 에이전트 수
agentcrow status              # 상태 확인 (프로젝트 + 글로벌)
agentcrow off [--global]      # 임시 비활성화
agentcrow on [--global]       # 다시 활성화
agentcrow agents              # 전체 에이전트 목록
agentcrow agents search ai    # 키워드 검색
